...
awaitable<int> proc_task(bool for_all)
{
	...
	if (for_all)
		co_await guard::wait_for_all(t0, t1);
	else
		co_await guard::wait_for_any(t0, t1);
...
awaitable<void> co_two_task()
{
	...
	co_await sleep_for(1);
	co_await guard::wait_for_any(co_completed(2), wait_for_all(co_incompleted(3), co_incompleted(3)));
	LOG_DBG("arrived co_two_task success");
}
awaitable<void> co_two_task2()
{
	LOG_DBG("arrived co_two_task2");
	co_await guard::wait_for_any(co_incompleted(2), wait_for_all(co_completed(1), co_completed(1)));
...
awaitable<void> co_three_task3()
{
	...
	co_await sleep_for(1);
	co_await guard::wait_for_any(co_incompleted(2), co_incompleted(2), co_completed(1));
...
awaitable<void> co_three_task4()
{
	...
	LOG_DBG("arrived co_three_task3");
	co_await sleep_for(1);
	co_await guard::wait_for_all(co_completed(2), co_completed(2), co_completed(1));
