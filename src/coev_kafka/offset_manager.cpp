...
namespace coev::kafka
{
    ...
    awaitable<int> OffsetManager::create_partition_offset_manager(const std::string &topic, int32_t partition, std::shared_ptr<PartitionOffsetManager> &pom)
    {
    ...
    awaitable<int> OffsetManager::close()
    {
    ...
    awaitable<int> OffsetManager::fetch_initial_offset(const std::string &topic, int32_t partition, int retries, int64_t &offset, int32_t &leaderEpoch, std::string &metadata)
    {
    ...
    awaitable<int> OffsetManager::coordinator(std::shared_ptr<Broker> &broker)
    {
    ...
    awaitable<void> OffsetManager::main_loop()
    {
    ...
    awaitable<int> OffsetManager::commit()
    {
        co_await flush_to_broker();
        release_poms(false);
        co_return 0;
    ...
    awaitable<void> OffsetManager::flush_to_broker()
    {
        std::shared_ptr<Broker> broker;
        auto err = co_await coordinator(broker);
        if (err != ErrNoError)
