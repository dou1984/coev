...
namespace coev::kafka
{
    ...
    struct ConsumerGroupSession : std::enable_shared_from_this<ConsumerGroupSession>
    {
        ...
        awaitable<void> hander_error(std::shared_ptr<PartitionOffsetManager> pom, const std::string &topic, int32_t partition);

        awaitable<int> _release(bool withCleanup);
        awaitable<void> _consume(const std::string &topic, int32_t partition);
        awaitable<void> _heartbeat_loop();
    };
}