...
namespace coev::kafka
{
    ...
    struct OffsetManager : std::enable_shared_from_this<OffsetManager>
    {
        ...
        awaitable<int> create_partition_offset_manager(const std::string &topic, int32_t partition, std::shared_ptr<PartitionOffsetManager> &pom);
        awaitable<int> close();
        awaitable<int> commit();
        ...
        awaitable<int> fetch_initial_offset(const std::string &topic, int32_t partition, int retries, int64_t &offset, int32_t &leaderEpoch, std::string &metadata);
        awaitable<int> coordinator(std::shared_ptr<Broker> &);
        awaitable<void> main_loop();
        awaitable<void> flush_to_broker();
