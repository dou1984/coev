...
namespace coev::kafka
{
    ...
    awaitable<int> AsyncProducer::producer(std::shared_ptr<ProducerMessage> msg, std::shared_ptr<ProducerMessage> &reply)
    {
        m_input.set(msg);
        co_await m_replies.get(reply);
        co_return ErrNoError;
    }
    awaitable<int> AsyncProducer::close()
    {
        async_close();
    ...
    awaitable<int> AsyncProducer::commit_txn()
    {
        if (!is_transactional())
        {
    ...
    awaitable<int> AsyncProducer::abort_txn()
    {
        if (!is_transactional())
        {
    ...
    awaitable<int> AsyncProducer::finish_transaction(bool commit)
    {
        m_in_flight.add();
    ...
    awaitable<void> AsyncProducer::dispatcher()
    {
        while (true)
        {
            std::shared_ptr<ProducerMessage> msg;
    ...
    awaitable<void> AsyncProducer::retry_handler()
    {
    ...
    awaitable<void> AsyncProducer::shutdown()
    {
        m_in_flight.add();
        auto msg = std::make_shared<ProducerMessage>();
    ...
    awaitable<void> AsyncProducer::bump_idempotent_producer_epoch()
    {
    ...
    awaitable<void> AsyncProducer::retry_batch(const std::string &topic, int32_t partition, std::shared_ptr<PartitionSet> partition_set, KError kerr)
    {
    ...
    awaitable<void> AsyncProducer::abandon_broker_connection(std::shared_ptr<Broker> broker)
    {
        bool ok;
    ...
    awaitable<int> NewAsyncProducer(const std::vector<std::string> &addrs, std::shared_ptr<Config> conf, std::shared_ptr<AsyncProducer> &producer)
    {
    ...
    awaitable<int> NewAsyncProducer(std::shared_ptr<Client> client, std::shared_ptr<AsyncProducer> &producer)
    {
        if (client->Closed())
        {
            co_return ErrClosedClient;
