...
coev::awaitable<int> Client::_GetReplicas(const std::string &topic, int32_t partition_id, std::shared_ptr<PartitionMetadata> &out)
{
...
coev::awaitable<int> Client::ResolveCanonicalNames(const std::vector<std::string> &addrs, std::vector<std::string> &ips)
{
    ...
    for (auto i = 0; i < addrs.size(); ++i)
    {
        ...
        task << [](auto &addr, auto &ip) -> coev::awaitable<void>
        {
