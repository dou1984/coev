...
namespace coev::kafka
{
    ...
    awaitable<int> GSSAPIKerberosAuth::WritePackage(std::shared_ptr<Broker> broker, const std::string &payload, int &bytes_written)
    {

        co_return ErrNoError;
    }

    awaitable<int> GSSAPIKerberosAuth::ReadPackage(std::shared_ptr<Broker> broker, std::string &payload, int &bytes_read)
    {
    ...
    awaitable<int> GSSAPIKerberosAuth::Login(std::shared_ptr<KerberosClient> client, const std::string &spn, std::shared_ptr<Ticket> &ticket_out)
    {

        co_return ErrNoError;
    }

    awaitable<int> GSSAPIKerberosAuth::Authorize(std::shared_ptr<Broker> broker)
    {
        co_return ErrNoError;
    }

    awaitable<int> GSSAPIKerberosAuth::AuthorizeV2(std::shared_ptr<Broker> broker, AuthSendReceiver authSendReceiver)
    {
        co_return ErrNoError;
    }
}