...
namespace coev::kafka
{
    ...
    awaitable<void> BrokerConsumer::SubscriptionManager()
    {
    ...
    awaitable<void> BrokerConsumer::SubscriptionConsumer()
    {
    ...
    awaitable<void> BrokerConsumer::HandleResponses()
    {
        for (auto it = m_subscriptions.begin(); it != m_subscriptions.end();)
        {
            auto child = *it;
    ...
    awaitable<void> BrokerConsumer::Abort(int err)
    {
    ...
    awaitable<int> BrokerConsumer::FetchNewMessages(std::shared_ptr<FetchResponse> &response)
    {
    ...
    awaitable<int> BrokerConsumer::Partitions(const std::string &topic, std::vector<int32_t> &out)
    {
        return m_consumer->Partitions(topic, out);
    }
    awaitable<int> BrokerConsumer::Close()
    {
