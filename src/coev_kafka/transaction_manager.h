...
namespace coev::kafka
{
    ...
    struct TransactionManager
    {
        ...
        awaitable<int> publish_offsets_to_txn(const TopicPartitionOffsets &offsets, const std::string &groupId, TopicPartitionOffsets &outOffsets);
        awaitable<Result> publish_offsets_to_txn_add_offset(const std::string &groupId);
        awaitable<Result> publish_offsets_to_txn_commit(const TopicPartitionOffsets &offsets, const std::string &groupId, TopicPartitionOffsets &outOffsets);
        awaitable<int> init_producer_id(int64_t &producer_id, int16_t &producer_epoch);
        int abortable_error_if_possible(int err);
        int complete_transaction();

        awaitable<int> end_txn(bool commit);
        awaitable<int> finish_transaction(bool commit);
        awaitable<int> publish_txn_partitions();
        awaitable<int> initialize_transactions();
        ...
        awaitable<Result> _end_txn(bool commit);
        awaitable<Result> _publish_txn_partitions();
