...
TEST(MetadataProtocolTest, TestMetadataResponseEncodeDecode) {
    ...
    // Add a broker
    original.AddBroker("localhost:9092", 1);
    
    // Add a topic with a partition
    original.AddTopicPartition(
    ...
    if (!original.m_topics.empty() && !decoded.m_topics.empty()) {
        EXPECT_EQ(decoded.m_topics[0]->Name, original.m_topics[0]->Name);
        EXPECT_EQ(decoded.m_topics[0]->Partitions.size(), original.m_topics[0]->Partitions.size());
    }
}
...
TEST(MetadataProtocolTest, TestMetadataResponseMultipleTopics) {
    ...
    // Add multiple topics
    original.AddTopicPartition("topic-1", 0, 1, {1}, {1}, {}, 0);
    original.AddTopicPartition("topic-1", 1, 1, {1}, {1}, {}, 0);
    original.AddTopicPartition("topic-2", 0, 1, {1}, {1}, {}, 0);
    ...
    EXPECT_EQ(decoded.m_topics[0]->Partitions.size(), 2); // topic-1 has 2 partitions
    EXPECT_EQ(decoded.m_topics[1]->Partitions.size(), 1); // topic-2 has 1 partition
}
...
TEST(MetadataProtocolTest, TestMetadataResponseVersionCompatibility) {
    ...
    for (int16_t version = 0; version <= 10; ++version) {
        ...
        // Add a broker
        original.AddBroker("localhost:9092", version);
...
TEST(MetadataProtocolTest, TestMetadataResponseWithMultipleBrokers) {
    ...
    original.AddBroker("broker1:9092", 1);
    original.AddBroker("broker2:9092", 2);
    original.AddBroker("broker3:9092", 3);
    
    // Add a topic with multiple partitions
    original.AddTopicPartition("test-topic", 0, 1, {1, 2, 3}, {1, 2}, {3}, 0);
    original.AddTopicPartition("test-topic", 1, 2, {1, 2, 3}, {2, 3}, {1}, 0);
    original.AddTopicPartition("test-topic", 2, 3, {1, 2, 3}, {1, 3}, {2}, 0);
    ...
    EXPECT_EQ(decoded.m_topics[0]->Partitions.size(), 3);
    
    // Verify some partition details
    EXPECT_EQ(decoded.m_topics[0]->Partitions[0]->m_leader, 1);
    EXPECT_EQ(decoded.m_topics[0]->Partitions[1]->m_leader, 2);
    EXPECT_EQ(decoded.m_topics[0]->Partitions[2]->m_leader, 3);
    
    // Verify replicas and ISR
    EXPECT_EQ(decoded.m_topics[0]->Partitions[0]->m_replicas.size(), 3);
    EXPECT_EQ(decoded.m_topics[0]->Partitions[0]->m_isr.size(), 2);
    EXPECT_EQ(decoded.m_topics[0]->Partitions[0]->m_offline_replicas.size(), 1);
}
