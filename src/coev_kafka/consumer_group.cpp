...
namespace coev::kafka
{
    ...
    awaitable<int> ConsumerGroup::Consume(std::shared_ptr<Context> &context, const std::vector<std::string> &topics, std::shared_ptr<ConsumerGroupHandler> handler)
    {
        bool is_closed;
        co_await m_closed.get(is_closed);
        if (is_closed)
        {
        ...
        auto err = co_await NewSession(context, topics, handler, m_config->Consumer.Group.Rebalance.Retry.Max, session);
        if (err != ErrNoError)
        {
            co_return err;
        }
    ...
    awaitable<int> ConsumerGroup::Close()
    {
        bool isClosed;
    ...
    awaitable<int> ConsumerGroup::RetryNewSession(std::shared_ptr<Context> &ctx, const std::vector<std::string> &topics, std::shared_ptr<ConsumerGroupHandler> handler, int retries, bool refreshCoordinator, std::shared_ptr<ConsumerGroupSession> &out)
    {
        coev::co_task _task;
        auto __get = _task << [](auto ctx) -> awaitable<void>
        {
            bool dummy;
            co_await ctx->ch.get(dummy);
            co_return;
        }(ctx);
        auto __close = _task << [](auto _this) -> awaitable<void>
        {
            bool dummy;
            co_await _this->m_closed.get(dummy);
            co_return;
        }(shared_from_this());
        auto __config = _task << [](auto _this) -> awaitable<void>
        {
    ...
    awaitable<int> ConsumerGroup::NewSession(std::shared_ptr<Context> &context, const std::vector<std::string> &topics, std::shared_ptr<ConsumerGroupHandler> handler, int retries, std::shared_ptr<ConsumerGroupSession> &session)
    {
        ...
        if (err != 0)
        {
            ...
            co_return co_await RetryNewSession(context, topics, handler, retries, true, session);
        }
        ...
        case ErrIllegalGeneration:
            m_member_id = "";
            co_return co_await NewSession(context, topics, handler, retries, session);
        ...
        err = co_await NewConsumerGroupSession(context, shared_from_this(), claims, response->m_member_id, response->m_generation_id, handler, session);
        if (err != 0)
        {
            co_return err;
        }
    ...
    awaitable<int> ConsumerGroup::JoinGroup(std::shared_ptr<Broker> coordinator, const std::vector<std::string> &topics, std::shared_ptr<JoinGroupResponse> &response)
    {
        ...
        {
            for (const auto &strategy : m_config->Consumer.Group.Rebalance.GroupStrategies)
            {
                err = request->add_group_protocol_metadata(strategy->Name(), meta);
                if (err != 0)
                {
    ...
    awaitable<int> ConsumerGroup::SyncGroup(std::shared_ptr<Broker> coordinator, std::map<std::string, ConsumerGroupMemberMetadata> members,
    ...
    awaitable<int> ConsumerGroup::Heartbeat(std::shared_ptr<Broker> coordinator, const std::string &memberID, int32_t generationID,
    ...
    awaitable<int> ConsumerGroup::Balance(std::shared_ptr<BalanceStrategy> strategy,
    ...
    awaitable<int> ConsumerGroup::Leave()
    {
    ...
    awaitable<void> ConsumerGroup::LoopCheckPartitionNumbers(
        ...
        const std::vector<std::string> &topics, std::shared_ptr<ConsumerGroupSession> &session)
    {
        ...
        while (true)
        {
            ...
            m_task << [this]() -> awaitable<void>
            {
                co_await sleep_for(m_config->Metadata.RefreshFrequency);
            }();
            m_task << [](auto session) -> awaitable<void>
            {
                bool dummy;
    ...
    awaitable<int> ConsumerGroup::TopicToPartitionNumbers(const std::vector<std::string> &topics, std::map<std::string, int> &topicToPartitionNum)
    {
    ...
    awaitable<int> NewConsumerGroup(const std::vector<std::string> &addrs, const std::string &group_id, std::shared_ptr<Config> config, std::shared_ptr<ConsumerGroup> &consumer_group)
    {
    ...
    awaitable<int> NewConsumerGroupSession(std::shared_ptr<Context> context, std::shared_ptr<ConsumerGroup> parent, std::map<std::string, std::vector<int32_t>> &claims, const std::string &memberID, int32_t generationID, std::shared_ptr<ConsumerGroupHandler> &handler, std::shared_ptr<ConsumerGroupSession> &session)
    {

        std::function<void()> cancel;
        std::shared_ptr<OffsetManager> offsets = std::make_shared<OffsetManager>(parent->m_client, parent->m_group_id, "", -1, cancel);
        session = std::make_shared<ConsumerGroupSession>(parent, memberID, generationID, handler, offsets, claims, context, cancel);

        for (auto &[topic, partitions] : claims)
        {

            for (auto partition : partitions)
            {
                auto pom = offsets->manage_partition(topic, partition);
                if (pom == nullptr)
                {
                ...
                session->m_task << [](auto session, auto pom, auto &topic, auto &partition) -> awaitable<void>
                {
                    while (true)
                    {
                    ...
                    co_return;
                }(session, pom, topic, partition);
                session->m_task << session->hander_error(pom, topic, partition);
            }
        }

        auto err = handler->setup(session);
        if (err != ErrNoError)
        {
        ...
        for (auto &[topic, partitions] : claims)
        {
            ...
            for (auto &partition : partitions)
            {
                session->m_task << [](auto session, auto &topic, auto &partition) -> awaitable<void>
                {
