...
namespace coev::kafka
{
    ...
    awaitable<int> ClusterAdmin::GetController(std::shared_ptr<Broker> &out)
    {
        return m_client->Controller(out);
    }

    awaitable<int> ClusterAdmin::GetCoordinator(const std::string &group, std::shared_ptr<Broker> &out)
    {
        return m_client->GetCoordinator(group, out);
    }
    awaitable<int> ClusterAdmin::RefreshController(std::shared_ptr<Broker> &out)
    {
    ...
    awaitable<int> ClusterAdmin::_CreateTopic(const std::string &topic, std::shared_ptr<CreateTopicsRequest> request)
    {
    ...
    awaitable<int> ClusterAdmin::CreateTopic(const std::string &topic, const std::shared_ptr<TopicDetail> &detail, bool validate_only)
    {
        if (topic.empty())
        {
    ...
    awaitable<int> ClusterAdmin::_DescribeTopics(const std::vector<std::string> &topics, std::vector<TopicMetadata> &out)
    {
    ...
    awaitable<int> ClusterAdmin::DescribeTopics(const std::vector<std::string> &topics, std::vector<TopicMetadata> &out)
    {
        co_return co_await RetryOnError(isRetriableControllerError, &ClusterAdmin::_DescribeTopics, topics, out);
    }

    awaitable<int> ClusterAdmin::_DescribeCluster(std::vector<std::shared_ptr<Broker>> &out, int32_t &out_controller_id)
    {
        std::shared_ptr<Broker> controller;
    ...
    awaitable<int> ClusterAdmin::DescribeCluster(std::vector<std::shared_ptr<Broker>> &brokers, int32_t &controller_id)
    {
    ...
    awaitable<int> ClusterAdmin::ListTopics(std::map<std::string, TopicDetail> &topics_details_map)
    {
        std::shared_ptr<Broker> broker;
    ...
    awaitable<int> ClusterAdmin::_DeleteTopic(const std::string &topic, std::shared_ptr<DeleteTopicsRequest> request)
    {
        std::shared_ptr<Broker> broker;
        int err = co_await GetController(broker);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::DeleteTopic(const std::string &topic)
    {
        if (topic.empty())
        {
            co_return ErrInvalidTopic;
    ...
    awaitable<int> ClusterAdmin::_CreatePartitions(const std::string &topic, std::shared_ptr<CreatePartitionsRequest> request)
    {
        std::shared_ptr<Broker> broker;
        int err = co_await GetController(broker);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::CreatePartitions(const std::string &topic, int32_t count, const std::vector<std::vector<int32_t>> &assignment, bool validate_only)
    {
        if (topic.empty())
        {
            co_return ErrInvalidTopic;
    ...
    awaitable<int> ClusterAdmin::_AlterPartitionReassignments(std::shared_ptr<AlterPartitionReassignmentsRequest> request)
    {
        std::shared_ptr<Broker> broker;
        int err = co_await GetController(broker);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::AlterPartitionReassignments(const std::string &topic, const std::vector<std::vector<int32_t>> &assignment)
    {
        if (topic.empty())
        {
            co_return ErrInvalidTopic;
    ...
    awaitable<int> ClusterAdmin::_ListPartitionReassignments(std::shared_ptr<ListPartitionReassignmentsRequest> request, std::map<std::string, std::map<int32_t, PartitionReplicaReassignmentsStatus>> &output)
    {
        std::shared_ptr<Broker> broker;
        int err = co_await GetController(broker);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::ListPartitionReassignments(const std::string &topic, const std::vector<int32_t> &partitions, std::map<std::string, std::map<int32_t, PartitionReplicaReassignmentsStatus>> &output)
    {
        if (topic.empty())
        {
            co_return ErrInvalidTopic;
    ...
    awaitable<int> ClusterAdmin::DeleteRecords(const std::string &topic, const std::map<int32_t, int64_t> &partition_offsets)
    {
        if (topic.empty())
        {
            co_return ErrInvalidTopic;
    ...
    awaitable<int> ClusterAdmin::DescribeConfig(const ConfigResource &cfg_resource, std::vector<ConfigEntry> &out)
    {
    ...
    awaitable<int> ClusterAdmin::IncrementalAlterConfig(ConfigResourceType resource_type, const std::string &name, const std::map<std::string, IncrementalAlterConfigsEntry> &entries, bool validate_only)
    {
    ...
    awaitable<int> ClusterAdmin::CreateACL(const Resource &resource, const Acl &acl)
    {
    ...
    awaitable<int> ClusterAdmin::DeleteACL(const AclFilter &filter, bool validate_only, std::vector<MatchingAcl> &out)
    {
    ...
    awaitable<int> ClusterAdmin::_ElectLeaders(std::shared_ptr<ElectLeadersRequest> request, ResponsePromise<ElectLeadersResponse> &res)
    {
        std::shared_ptr<Broker> broker;
        int err = co_await GetController(broker);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::ElectLeaders(ElectionType electionType, const std::unordered_map<std::string, std::vector<int32_t>> &partitions, std::unordered_map<std::string, std::map<int32_t, PartitionResult>> &out)
    {
    ...
    awaitable<int> ClusterAdmin::DescribeConsumerGroups(const std::vector<std::string> &groups, std::vector<GroupDescription> &out)
    {
    ...
    awaitable<int> ClusterAdmin::ListConsumerGroups(std::map<std::string, std::string> &out)
    {
        auto brokers = m_client->Brokers();
    ...
    awaitable<int> ClusterAdmin::_ListConsumerGroups(std::shared_ptr<Broker> broker, std::list<int> &err_chan, std::map<std::string, std::string> &out)
    {
        broker->Open(m_conf);
    ...
    awaitable<int> ClusterAdmin::_ListConsumerGroupOffsets(const std::string &group, std::shared_ptr<OffsetFetchRequest> request, std::shared_ptr<OffsetFetchResponse> &out)
    {
        std::shared_ptr<Broker> coordinator;
        int err = co_await m_client->GetCoordinator(group, coordinator);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::_DeleteConsumerGroupOffset(std::shared_ptr<DeleteOffsetsRequest> request, const std::string &group, const std::string &topic, int32_t partition)
    {
        std::shared_ptr<Broker> coordinator;
        int err = co_await m_client->GetCoordinator(group, coordinator);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::DeleteConsumerGroupOffset(const std::string &group, const std::string &topic, int32_t partition)
    {
    ...
    awaitable<int> ClusterAdmin::_DeleteConsumerGroup(std::shared_ptr<DeleteGroupsRequest> request, const std::string &group)
    {
        std::shared_ptr<Broker> coordinator;
        int err = co_await m_client->GetCoordinator(group, coordinator);
        if (err != 0)
    ...
    awaitable<int> ClusterAdmin::DeleteConsumerGroup(const std::string &group)
    {
                                                       ...
                                                       std::map<int32_t, std::vector<DescribeLogDirsResponseDirMetadata>> &out)
    {
        ...
        for (int32_t bid : broker_ids)
        {
            ...
            task << [this](auto broker, auto &log_dirs_results, auto &err_chan) -> awaitable<void>
            {
    ...
    awaitable<int> ClusterAdmin::DescribeUserScramCredentials(const std::vector<std::string> &users, std::vector<DescribeUserScramCredentialsResult> &out_results)
    {
    ...
    awaitable<int> ClusterAdmin::DescribeClientQuotas(const std::vector<QuotaFilterComponent> &components, bool strict, std::vector<DescribeClientQuotasEntry> &out)
    {
    ...
    awaitable<int> ClusterAdmin::AlterClientQuotas(const std::vector<QuotaEntityComponent> &entity, const ClientQuotasOp &op, bool validate_only)
    {
    ...
    awaitable<int> ClusterAdmin::_RemoveMemberFromConsumerGroup(std::shared_ptr<LeaveGroupRequest> request, std::shared_ptr<LeaveGroupResponse> &response, const std::string &groupId)
    {
        std::shared_ptr<Broker> coordinator;
    ...
    awaitable<int> ClusterAdmin::RemoveMemberFromConsumerGroup(const std::string &groupId, const std::vector<std::string> &groupInstanceIds, std::shared_ptr<LeaveGroupResponse> &response)
    {
        if (!m_conf->Version.IsAtLeast(V2_4_0_0))
        {
