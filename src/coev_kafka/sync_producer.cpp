...
namespace coev::kafka
{
    ...
    awaitable<int> SyncProducer::send_message(std::shared_ptr<ProducerMessage> msg, int32_t &partition, int64_t &offset)
    {
    ...
    awaitable<int> SyncProducer::send_messages(const std::vector<std::shared_ptr<ProducerMessage>> &msgs)
    {

        coev::co_task task;
        task << [this](auto size) -> awaitable<void>
        {
    ...
    awaitable<void> SyncProducer::handle_replies()
    {
        while (!closed)
        {
            std::shared_ptr<ProducerMessage> msg;
    ...
    awaitable<int> SyncProducer::commit_txn()
    {
        return m_producer->commit_txn();
    }

    awaitable<int> SyncProducer::abort_txn()
    {
    ...
    awaitable<int> NewSyncProducer(const std::vector<std::string> &addrs, std::shared_ptr<Config> &config, std::shared_ptr<SyncProducer> &producer)
    {
        producer = nullptr;
        if (config == nullptr)
        {
    ...
    awaitable<int> NewSyncProducerFromClient(std::shared_ptr<Client> client, std::shared_ptr<SyncProducer> &producer)
    {
