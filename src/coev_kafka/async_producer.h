...
namespace coev::kafka
{
    ...
    struct AsyncProducer : std::enable_shared_from_this<AsyncProducer>
    {
        ...
        awaitable<int> producer(std::shared_ptr<ProducerMessage> msg, std::shared_ptr<ProducerMessage> &reply);
        ...
        awaitable<void> dispatcher();
        awaitable<void> retry_handler();
        awaitable<void> shutdown();
        awaitable<void> bump_idempotent_producer_epoch();
        ...
        awaitable<void> abandon_broker_connection(std::shared_ptr<Broker> broker);
        awaitable<int> finish_transaction(bool commit);
        awaitable<void> retry_batch(const std::string &topic, int32_t partition, std::shared_ptr<PartitionSet> pSet, KError kerr);
