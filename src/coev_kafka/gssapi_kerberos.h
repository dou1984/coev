...
namespace coev::kafka
{
    ...
    struct GSSAPIKerberosAuth
    {
        ...
        awaitable<int> WritePackage(std::shared_ptr<Broker> broker, const std::string &payload, int &bytes_written);
        awaitable<int> ReadPackage(std::shared_ptr<Broker> broker, std::string &payload, int &bytes_read);
        ...
        awaitable<int> Login(std::shared_ptr<KerberosClient> client, const std::string &spn, std::shared_ptr<Ticket> &ticket_out);
        awaitable<int> Authorize(std::shared_ptr<Broker> broker);
        awaitable<int> AuthorizeV2(std::shared_ptr<Broker> broker, AuthSendReceiver authSendReceiver);
    };
}