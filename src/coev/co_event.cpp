/*
 *	coev - c++20 coroutine library
 *
 *	Copyright (c) 2023-2026, Zhao Yun Shan
 *
 */
#include <thread>
#include <utility>
#include "co_event.h"
#include "local_resume.h"

namespace coev
{
	void co_event::__set_reserved(uint64_t x)
	{
		m_reserved = x;
	}
	uint64_t co_event::__get_reserved()
	{
		return m_reserved;
	}
	co_event::co_event(queue *__ev) : m_tid(gtid())
	{
		__ev->push_back(this);
	}

	co_event::~co_event()
	{
		if (!queue::empty())
		{
			queue::erase(this);
		}
		m_caller = nullptr;
	}
	uint64_t co_event::await_resume()
	{
		return __get_reserved();
	}
	bool co_event::await_ready()
	{
		return m_status.load(std::memory_order_consume);
	}
	void co_event::await_suspend(std::coroutine_handle<> _caller)
	{
		m_caller = _caller;
		int e0 = details::CORO_INIT;
		int e1 = details::CORO_RESUMED;
		if (m_status.compare_exchange_strong(e0, details::CORO_SUSPEND, std::memory_order_acquire, std::memory_order_acquire))
		{
		}
		else if (m_status.compare_exchange_strong(e1, details::CORO_FINISHED, std::memory_order_acquire, std::memory_order_acquire))
		{
			__resume();
		}
		else
		{
			assert(false);
		}
	}
	void co_event::resume()
	{
		int e0 = details::CORO_INIT;
		int e1 = details::CORO_SUSPEND;
		if (m_status.compare_exchange_strong(e0, details::CORO_RESUMED, std::memory_order_acquire, std::memory_order_acquire))
		{
		}
		else if (m_status.compare_exchange_strong(e1, details::CORO_FINISHED, std::memory_order_acquire, std::memory_order_acquire))
		{
			__resume();
		}
		else
		{
			assert(false);
		}
	}
	void co_event::__resume()
	{
		if (m_caller && m_caller.address() && !m_caller.done())
		{
			auto _caller = std::exchange(m_caller, nullptr);
			_caller.resume();
		}
		else
		{
			assert(false);
		}
	}
}
